...
import { ArrowLeft, Plus, Edit, Download, Trash2, DollarSign } from 'lucide-react';
...
import { calculatePaymentStatus, calculateDepositStatus, formatCurrency } from '@/lib/types';
...
export default function RentalDetail() {
  ...
  const [selectedPaymentForQuickPay, setSelectedPaymentForQuickPay] = useState<any>(null);
  ...
  const handleQuickPayPayment = (payment: any) => {
  ...
  const handleQuickPayTotal = () => {
    if (selectedPaymentForQuickPay) {
      console.log('Quick pay total:', selectedPaymentForQuickPay);
      // TODO: Update payment status to paid
      setQuickPaymentModalOpen(false);
    }
  };

  const handleQuickPayPartial = (amount: number) => {
    if (selectedPaymentForQuickPay) {
      console.log('Quick pay partial:', amount, selectedPaymentForQuickPay);
  ...
  const handleAddDeposit = (data: any) => {
    console.log('Add deposit:', data);
    setDepositModalOpen(false);
    // TODO: Implement deposit addition
  };
  ...
  const handleShowDepositReceipt = (deposit: any, depositPayment: any) => {
    setSelectedReceipt({
      type: 'deposit',
      clientName: `${client.firstName} ${client.lastName}`,
      propertyName: rental.propertyName,
  ...
  const depositRemaining = rental.deposit.total - rental.deposit.paid;

  // Calculate payment statistics
  const paymentStats = {
    total: rental.payments.length,
    paid: rental.payments.filter((p: any) => calculatePaymentStatus(p) === 'paid').length,
    partial: rental.payments.filter((p: any) => calculatePaymentStatus(p) === 'partial').length,
    unpaid: rental.payments.filter((p: any) => {
      const status = calculatePaymentStatus(p);
      return status === 'unpaid' || status === 'late';
    }).length,
    totalAmount: rental.payments.reduce((sum: number, p: any) => sum + p.amount, 0),
    paidAmount: rental.payments.reduce((sum: number, p: any) => sum + p.paidAmount, 0),
  };

  const paymentStats_remaining = paymentStats.totalAmount - paymentStats.paidAmount;
              ...
              {rental.deposit.payments && rental.deposit.payments.length > 0 ? (
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {rental.deposit.payments.map((payment: any) => (
                        <TableRow key={payment.id}>
                          <TableCell>
